...
class MyUtil
{
	...
	int incorrectAnswers(char[]answer,char[]student_answer){
		...
		//return 10-st.correctAnswers(answer,student_answer); 만약 다른 클래스에 있으면 이렇게 주면됨 
...
class Exam 
{
	public static void main(String[] args) 
	{
		...
		"터키의 수도는?"};
		String []view = {
			"a)아크라 b)서울 c)코팬어겐 d) 야렌",
			"a)워싱턴 b)서울 c)코팬어겐 d) 야렌",
			"a)아크라 b)서울 c)모나코 d) 야렌",
			"a)아크라 b)서울 c)코팬어겐 d) 브라질리아",
			"a)베른 b)서울 c)코팬어겐 d) 야렌",
			"a)아크라 b)싱가포르 c)코팬어겐 d) 야렌",
			"a)아크라 b)런던 c)코팬어겐 d) 야렌",
			"a)뉴델리 b)서울 c)코팬어겐 d) 야렌",
			"a)아크라 b)서울 c)오타와 d) 야렌",
			"a)앙카라 b)서울 c)코팬어겐 d) 야렌"};
